...
class CapsuleRepository {
  ...
  listCapsules() {
    return [...this.store.capsules].sort((a, b) => String(b.updatedAt || '').localeCompare(String(a.updatedAt || '')));
  }
...
class BridgeRegistry {
  ...
  async ensureBridge(globalName, targetNotePath = null) {
...
class CapsuleManagerModal extends Modal {
  ...
  async refresh() {
    ...
    helper.setText('Bind "State Capsules: Open capsule manager modal" to a hotkey for quick access.');

    const capsules = this.plugin.repository.listCapsules();
    const list = wrap.createDiv({ cls: 'state-capsules-list' });
    if (capsules.length === 0) {
      const empty = list.createDiv({ cls: 'state-capsules-empty' });
      empty.createEl('div', { text: 'No capsules saved yet.' });
      empty.createEl('small', { text: 'Create one from your current workspace state.' });
    } else {
      capsules.forEach((capsule) => {
        const isActive = capsule.id === this.plugin.repository.store.activeCapsuleId;
        ...
        headText.createEl('div', {
          text: `${capsule.updatedAt || ''}${isActive ? ' | active' : ''}`,
          cls: 'state-capsules-card-meta',
        });

        const detail = card.createDiv({ cls: 'state-capsules-card-detail' });
        const workspaceTab = capsule.workspaceMount?.activeTabId || '-';
        const gtdFilters = Array.isArray(capsule.gtd?.tagFilters) ? capsule.gtd.tagFilters.length : 0;
        detail.createEl('div', { text: `Workspace Tab: ${workspaceTab}` });
        detail.createEl('div', { text: `GTD Filters: ${gtdFilters}` });
        if (Array.isArray(capsule.meta?.warnings) && capsule.meta.warnings.length > 0) {
          detail.createEl('div', {
            text: `Warnings: ${capsule.meta.warnings.length}`,
            cls: 'state-capsules-warning',
          });
        }
        ...
        switchBtn.onclick = async () => {
          await this.plugin.restoreCapsuleById(capsule.id);
          await this.refresh();
        };
        ...
        renameBtn.onclick = async () => {
          await this.plugin.renameCapsuleById(capsule.id);
          await this.refresh();
        };
        ...
        deleteBtn.onclick = async () => {
          await this.plugin.deleteCapsuleById(capsule.id);
          await this.refresh();
        };
      });
    }
...
module.exports = class StateCapsulesPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.noteEventTracker.syncWithRepository();
  ...
  async overwriteActiveCapsule() {
    ...
    if (!ok) return;
    const next = this.assembler.captureCapsule(activeCapsule.name);
    next.id = activeCapsule.id;
    next.createdAt = activeCapsule.createdAt;
    next.updatedAt = nowString();
    await this.repository.updateCapsule(activeCapsule.id, next, { setActive: false });
    await this.refreshViews();
