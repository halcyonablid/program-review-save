...
function scheduleSprintsForGoals(goals) {
    ...
    for (const goal of scheduledGoals) {
        ...
        for (const step of goal.steps) {
            ...
            let bestStart = findBestStartDate(goal, timeline, sprintDuration, currentDate);

            if (!bestStart || bestStart.getTime() + sprintDuration * 24 * 60 * 60 * 1000 > goal.dueDate.getTime()) {
                bestStart = new Date(Math.max(currentDate, new Date()));
            }
            ...
            if (step !== goal.steps[goal.steps.length - 1]) {
                const restEnd = new Date(currentDate.getTime() + 5 * 24 * 60 * 60 * 1000);
                if (restEnd <= goal.dueDate) {
                    goal.sprints.push({
...
function findBestStartDate(goal, timeline, duration, lastSprintEnd) {
