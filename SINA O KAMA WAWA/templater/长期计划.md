...
function scheduleSprintsForGoals(goals) {
    ...
    for (const goal of goals) {
        ...
        for (const step of goal.steps) {
            ...
            const bestStart = findNextAvailableStartDate(goal, step, timeline, sprintDuration, lastSprintEnd);
...
function findNextAvailableStartDate(goal, step, timeline, duration, lastSprintEnd) {
    let startDate = new Date(Math.max(goal.startDate, lastSprintEnd ? new Date(lastSprintEnd.getTime() + duration * 24 * 60 * 60 * 1000) : goal.startDate));
    let bestStart = startDate;
    let minLoad = Infinity;

    while (startDate <= goal.dueDate) {
        if (startDate.getTime() + duration * 24 * 60 * 60 * 1000 > goal.dueDate.getTime()) break;

        const load = calculateLoad(timeline, startDate, duration);
        if (load < minLoad) {
            minLoad = load;
            bestStart = new Date(startDate);
        }
