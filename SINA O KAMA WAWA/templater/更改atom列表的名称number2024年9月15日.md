...
if (currentFile && currentFile.name.startsWith("ZL@")) {
    ...
    // 使用 tp.frontmatter 获取 number 属性
    const numberValue = tp.frontmatter.number;
    ...
    // 更新文件名(如果存在 number 属性)
    if (numberValue !== undefined) {
        const currentName = currentFile.name;
        const newName = currentName.replace(/\.md$/, ` (${numberValue}).md`);
        await app.fileManager.renameFile(currentFile, newName);
        new Notice(`已更新文件 ${newName} 的哈希值为 ${hashValue},并添加了编号 (${numberValue})`, 3000);
    } else {
