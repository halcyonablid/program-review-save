...
async function openPreviousStructuredFile() {
  try {
    // 1. 获取当前文件
    ...
    // 6. 确定前一个文件（如果是第一个文件则循环到最后一个）
    const previousIndex = (currentIndex - 1 + structuredFiles.length) % structuredFiles.length;
    const previousFile = structuredFiles[previousIndex];
    
    // 7. 打开前一个文件
    await app.workspace.openLinkText(previousFile.path, "", false);
    
    const fileType = previousFile.basename.startsWith("ATOM@") ? "ATOM" : "ZL";
   
  
  } catch (error) {
    console.error('打开前一个文件时出错:', error);
    new Notice(`打开前一个文件时出错：${error.message}`);
  }
}

// 运行主函数
await openPreviousStructuredFile();
%>