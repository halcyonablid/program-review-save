...
function scheduleSprintsForGoals(goals) {
    ...
    for (const goal of goals) {
        const sprints = [];
        let lastSprintEnd = new Date(goal.startDate);

        for (const step of goal.steps) {
            const sprintDuration = Math.min(step.duration, 14);
            const bestStart = findBestStartDate(goal, step, timeline, sprintDuration, lastSprintEnd);
...
async function updateYAMLFrontmatter(scheduledGoals) {
    for (const goal of scheduledGoals) {
        ...
        if (frontmatter) {
            const adjustedSprints = goal.sprints.map(sprint => {
            ...
            const updatedFrontmatter = { ...frontmatter, sprints: adjustedSprints };
